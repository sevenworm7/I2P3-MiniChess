...
Move submission::get_move(std::ofstream& fout){ //present state
  int ans_score = 0;
  Move ans_move;
  int tmp_score;
  ...
  for(int i = 0; i < (int)this->state->legal_actions.size(); i++){
    submission* target = new submission(this->state->next_state(this->state->legal_actions[i]));
...
int submission::extend(int depth){
